...
import { UserModel } from "../adapters/firebase/UserModel.ts";
...
export const fireOrmUserRepository = (): IUserRepository => {
  ...
  return {
    ...
    async update_vehicle_by_vehicle_slug_by_user_slug(_slug, plate, data) {
    ...
    async delete_vehicle_by_vehicle_slug_by_user_slug(_slug, plate) {
      const vehicle = await vehicleFireRepository
        .whereEqualTo("plate", plate)
        .findOne();
